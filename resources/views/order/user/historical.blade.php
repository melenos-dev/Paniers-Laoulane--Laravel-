@extends('layouts.app')
    @section('title')
        @lang('My order history') - Les paniers de Laoul'ane
    @endsection

    @section('middle')
        <div class="row">
            <div class="ariane">
                <a href="{{ route('user.index') }}">@lang('My account')</span></a> <i class="fa-solid fa-arrow-right"></i> <span class="active">@lang('My order history')</span>
            </div>
            ...
            <div class="col px-2 py-2">
                <h2>@lang('My order history')</h2>
                @if($orders)
                    <table class="table">
                        <thead>
                            <tr>
                                <th>@lang('Adressed to')</th>
                                <th>Date</th>
                                <th>Total</th>
                        ...
                        <tbody>
                            @foreach ($orders as $order)
                                <tr>
                                    ...
                                    <td align="right">
                                        <a href="{{ route('order.user.show', $order->id) }}">@lang('Open details')</a>
                                    </td>
                                </tr>
                            @endforeach
                        </tbody>
